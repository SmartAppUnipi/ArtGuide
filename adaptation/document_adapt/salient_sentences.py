...
class SalientSentence():
    def __init__(self, sentence, keyword, readibility, bpemb, stopwords = []):
        ...
        # create the embedding of the sentence
        self.sentence_rake_embed(stopwords, bpemb)
        # dictionary that associate to each keyword his embedding
        self.keyword = {k: bpemb.embed(k) for k in keyword}
        # dictionary that associate to each keyword the distance to the embedding
        self.keyword_distance = {k: np.mean(scipy.spatial.distance.cdist(self.keyword[k], self.sentence_embeddings, "cosine")) for k in keyword}
