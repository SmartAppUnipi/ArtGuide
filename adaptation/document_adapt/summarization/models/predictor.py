...
import os
import sys
sys.path.append('./document_adaptation/summarization/')
import math
...
class Translator(object):
    ...
    def translate(self,
                  data_iter, attn_debug=False):

        self.model.eval()

        pred_results, gold_results, src_out = [], [], []
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    ...
                    gold_results.append(gold_str + '\n')
                    src_out.append(src.strip() + '\n')   

        return pred_results, gold_results, src_out
    ...
    def _fast_translate_batch(self,
        ...
        if (batch.src_str):
            min_sent = min([len(l[0]) for l in batch.src_str])
            min_length = randint(int(min_sent/7), int(min_sent/4))*2
