...
async function parse(url: string) {
    return JSDOM.fromURL(url).then(dom => {
        ...
        do nodes = dom.window.document.querySelectorAll(querySelectors[i++])
        while (!nodes.length && i < querySelectors.length)
...
async function scrape (urls: string[]) {
    // TODO: specify an interface for result object
    return await Promise.all(urls.map(async url => ({ "url": url, "content": await parse(url) })))
}
