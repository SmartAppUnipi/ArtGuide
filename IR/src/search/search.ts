...
export class Search {
     ...
     */
    private buildBasicQueries(classificationResult: ClassificationResult): Query[] {
     ...
     */
    private extendQuery(queries: Query[],
                        queryExpansion: QueryExpansionResponse): Query[] {
     ...
     * @param classificationResult The object received from the Classification module.
     * @returns {Promise<Array<QueryBuildResult>>} A promise resolved with the array of 
     * queries of type QueryBuildResult.
     */
    private buildQueries(classificationResult: ClassificationResult): Promise<Query[]> {
     ...
     */
    private buildResult(queries: Query[]): Promise<PageResult[]> {

        /*
         * TODO:
         * For each basic query (eg. "Leaning Tower of Pisa") we perform a Google Search for each
         * keywords that come from the adaptation group 
        ...
        return Promise.all(
            ...
            queries.map(async q => {
                ...
                const expandedQuery = q.searchTerms + " " + q.keywords.join(" ");
                return this.googleSearch.queryCustom(expandedQuery).then(queryResult => {
