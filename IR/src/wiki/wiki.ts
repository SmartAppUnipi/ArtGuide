...
export class Wiki {
    ...
    public search(classificationResult: ClassificationResult): Promise<Array<PageResult>> {
        ...
        return Promise
            .all(
                queries.map(q => this.getWikiInfo(q.searchTerms, lang))
            )
            .catch(err => {
    ...
    /**
     * @param query The searched WikiData id
    ...
    private getPageByID(wikidata_id: string, language: string): Promise<Page> {
        return wiki({ apiUrl: "https://" + language + ".wikipedia.org/w/api.php" }).findById(wikidata_id)
            .catch(err => {
                logger.error("[wiki.ts] Error while retrieving Wikipedia page " + wikidata_id, err);
                throw err;
            });
    }
